...
- **Centralize Collections & Subcollections with Enums (`FirestoreCollection`, `FirestoreSubcollection`)**: Always use the `FirestoreCollection` and `FirestoreSubcollection` enums (from `data-model/collections.ts` or re-exported via `data-model/base.ts`) when calling `db.collection(...)` or defining database paths. Avoid raw string literals (`'members'`, `'events'`, `'registrations'`). This prevents typographical errors in database paths and provides a single source of truth for collection hierarchy.
- **Early Domain Typing on Firestore Snapshots (Anti-Pattern: Late Untyped Bracket Access `data()['field']`)**: When reading Firestore documents (`doc.data()` or `snap.data()`), cast the result to its authoritative domain type as early as possible (`snap.data() as IlcEvent | undefined`, `prodSnap.data() as Product | undefined`, `memberDoc.data() as Member | undefined`). Never leave snapshot data untyped or typed as `Record<string, unknown>`, and never access properties using index-signature bracket notation (`eventData['title']`). Early domain typing provides full compile-time validation, IDE autocomplete, prevents typos, and enables clean dot-property access (`event.title`).
- **Type Mutation / Update Accumulators with `Partial<T>` (or `MemberUpdates`)**: When building an update object to pass to Firestore `ref.update(updates)`, type it as `Partial<DomainType>` (e.g. `const updates: Partial<EventRegistration> = { ... }` or `MemberUpdates` for members). Never type update accumulators as `Record<string, unknown>`. Use dot notation (`updates.paymentStatus = ...`, `memberUpdates.membershipType = ...`) to prevent typos in field names and ensure all values match the domain schema.
