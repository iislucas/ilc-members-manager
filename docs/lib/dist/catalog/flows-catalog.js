...
exports.FLOWS_CATALOG = void 0;
const arch_flow_1 = require("../models/arch-flow");
exports.FLOWS_CATALOG = [
    {
        id: 'client-reactivity',
        title: 'Client UI Reactivity & Real-Time Snapshot Pipeline',
        category: arch_flow_1.FlowCategory.ClientReactivity,
        summary: 'How user gestures update Angular Signals, propagate mutations to Cloud Firestore, and reflect real-time onSnapshot changes without manual page refreshes.',
    ...
    },
    {
        ...
        category: arch_flow_1.FlowCategory.TriggerMirroring,
        summary: 'How database writes fire background Cloud Functions to maintain ACL caches, mirror rosters to school/instructor subcollections, and dispatch notifications.',
    ...
    },
    {
        ...
        category: arch_flow_1.FlowCategory.ECommerceWebhooks,
        summary: 'The flow from initiating a checkout session for memberships, tickets, or gradings through Stripe payment settlement, webhook validation, and automated idempotent fulfillment.',
    ...
    },
    {
        ...
        category: arch_flow_1.FlowCategory.MediaTranscoding,
        summary: 'Automated pipeline from master video file upload in GCS to GCP Transcoder API, multi-bitrate HLS generation, access token authorization, offline player caching, and admin direct access grants.',
    ...
    },
    {
        ...
        category: arch_flow_1.FlowCategory.MicroFrontends,
        summary: 'Packaging Angular components as standalone Web Components for embedding the event calendar and instructor directory on external affiliate websites.',
    ...
    },
    {
        ...
        category: arch_flow_1.FlowCategory.EmailAndNotifications,
        summary: 'Reliable transactional and scheduled email delivery pipeline featuring 3-state dispatch governance (Off, Paused, Active), zero-write on Off, placeholder queuing when Paused, deferred template interpretation upon activation, atomic locks against circular trigger loops, RFC 8058 one-click unsubscribe headers, and Google Workspace Gmail SMTP integration.',
        trigger: 'A transactional event occurs (Stripe purchase fulfillment, member onboarding), a scheduled cron triggers (weekly/monthly event digest across next 3 months), or an admin sends a test message.',
        steps: [
            {
                stepNumber: 1,
                sourceTier: 'Transactional Trigger / Dispatcher',
                targetTier: 'Status Check & /mail Queue',
                action: 'Evaluates global MailSendingStatus. If Off, logs and exits with zero writes. If Paused, enqueues placeholder with MailDeliveryState.Paused. If Active, renders template and writes MailDeliveryState.Pending. Admin tests bypass Off/Paused checks with metadata.adminTest: true.',
    ...
    },
    {
        ...
        category: arch_flow_1.FlowCategory.EmailAndNotifications,
        summary: 'Standardized RFC 8058 email unsubscribe handling supporting automated mail client HTTP POSTs (List-Unsubscribe=One-Click) and web browser GETs with HMAC-SHA256 signature verification and atomic member preference updates.',
        trigger: 'Recipient clicks the unsubscribe link in an outbound email footer or their email client triggers an automated one-click unsubscribe POST request.',
        steps: [
            {
                stepNumber: 1,
                sourceTier: 'Mail Client / Browser',
                targetTier: 'unsubscribeHandler HTTPS Endpoint',
                action: 'Issues HTTP POST (with List-Unsubscribe=One-Click header) or HTTP GET to /unsubscribeHandler with category (e.g. eventDigest, all) and HMAC token',
            ...
            },
            {
                ...
                action: 'Calls verifyUnsubscribeToken() retrieving persistent HMAC secret from /system/mail-settings and executes timing-safe comparison to prevent timing attacks',
            ...
            },
            {
                ...
                action: 'Locates member document and updates notification opt-out preferences (e.g. opt-out of event digest or all optional notifications)',
            ...
            },
            {
                ...
                action: 'Returns RFC 8058 compliant HTTP 200 OK for automated POSTs, or renders a branded HTML confirmation page with one-click resubscribe option and preferences link for GETs',
...
//# sourceMappingURL=flows-catalog.js.map