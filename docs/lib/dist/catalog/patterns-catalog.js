...
exports.PATTERNS_CATALOG = [
    ...
    },
    {
        ...
        problem: 'Multi-gigabyte video files (e.g. 8 GB seminar or class recordings) abort due to transient network drops or the Firebase Storage SDK default 10-minute retry limit (maxUploadRetryTime = 600000), causing storage/retry-limit-exceeded and forcing admins to start over from 0%.',
        solution: 'Leverage Google Cloud Storage native Resumable Upload protocol (uploadBytesResumable) with 256 KB chunking. Explicitly override SDK timeout on the storage instance to 24 hours (storage.maxUploadRetryTime = 86400000). Persist the unique GCS uploadUrl session in browser localStorage keyed by file.name + file.size + file.lastModified. When an upload is paused or interrupted, query the received byte offset via X-Goog-Upload-Command: query and resume from X-Goog-Upload-Size-Received without re-uploading completed bytes.',
        consequences: 'Allows multi-hour uploads on slower connections without timeout failures. Gives administrators pause/resume control and protects in-flight uploads with window beforeunload warnings. Sessions auto-expire after 7 days and clean up on completion.',
        canonicalCodePointers: [
    ...
    },
    {
        ...
        problem: 'If push notification preferences are stored solely in the cloud on the Member document, turning off notifications on a laptop mutes notifications on the user phone. Conversely, logging into a browser where OS permissions were previously granted would automatically resubscribe a device the user deliberately silenced.',
        solution: 'Decouple notification intent into two distinct scopes: (1) Account-wide preferences (globalPushEnabled, pushEnabled[kind]) stored on /members/{id} for backend Cloud Function dispatch filtering, and (2) Per-device hardware subscription state persisted in browser localStorage (devicePushEnabled). When devicePushEnabled is false, client Service Worker will never auto-subscribe or push tokens to the server, preventing unwanted background resubscription even if browser-level Notification permission remains granted.',
        consequences: 'Gives users granular hardware-level control over which physical devices receive alerts without corrupting shared cloud preferences or causing unexpected auto-resubscribes upon login.',
        canonicalCodePointers: [
