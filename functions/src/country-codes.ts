...
/**
 * Look up a country code from a country name or code string.
 * Matches by case-insensitive exact name, partial name (contains), or exact code.
 * Returns the country code (id) string, or null if not found.
 */
export function resolveCountryCode(input: string): string | null {
  ...
  // Then try partial name match (contains, case-insensitive)
  const byPartialName = countryCodeList.find(c => c.name.toLowerCase().includes(lower));
