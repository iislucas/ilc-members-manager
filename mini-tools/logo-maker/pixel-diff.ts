...
export let referenceImage: HTMLImageElement | null = null;

export function setReferenceImage(img: HTMLImageElement | null): void {
  referenceImage = img;
}

export function loadReferenceImage(src: string): Promise<HTMLImageElement> {
...
export async function updateDiff(p: LogoParams): Promise<{color: number, alpha: number} | undefined> {
  ...
  mathCanvas.width = diffSize; mathCanvas.height = diffSize;
...
export async function computeRMSEFast(p: LogoParams, maskParamKeys?: Array<keyof LogoParams>): Promise<{color: number, alpha: number}> {
